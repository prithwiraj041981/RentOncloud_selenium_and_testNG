...
public class AddContactspage extends TestBase{
	...
	@FindBy(how=How.XPATH, using="//i[@class='fi fi-rr-address-book	align-middle']")
	WebElement contacts;
	...
	public void addcontactofself() throws InterruptedException  {
		contacts.click();
		...
		addcontacts.click();
		Thread.sleep(TIMEOUT);
		Select dropdown3 = new Select(Contact_Type);
		Thread.sleep(TIMEOUT);
		List <WebElement> op3 = dropdown3.getOptions();
		Thread.sleep(TIMEOUT);
		 int size = op3.size();
		 for(int i =0; i<size ; i++){
	         String options = op3.get(i).getText();
	         System.out.println(options);
	      }
		 op3.get(2).click();
		 Thread.sleep(TIMEOUT);
		 logger.log(Status.PASS, MarkupHelper.createLabel(" User has select contact type", ExtentColor.GREEN));
		 Contact_email.sendKeys("prithwiraj021981@mailinator.com");
		 Thread.sleep(TIMEOUT);
